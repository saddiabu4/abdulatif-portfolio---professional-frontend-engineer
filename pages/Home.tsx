      ...
      <section className="flex flex-col lg:flex-row items-center justify-between gap-12 py-10 lg:py-24">
        <div className="w-full lg:w-2/3 space-y-6 lg:space-y-8 text-center lg:text-left">
          ...
          <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-4">
            <Link to="/projects" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold shadow-2xl shadow-blue-600/30 hover:bg-blue-700 transition-all hover:scale-105 active:scale-95 text-center">
              {t('hero.cta_work')}
            </Link>
            <Link to="/contact" className="px-10 py-5 border-2 border-slate-200 text-slate-900 rounded-2xl font-bold hover:bg-white hover:border-blue-600 transition-all text-center">
              {t('hero.cta_contact')}
            </Link>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/3 relative px-4 sm:px-10 lg:px-0">
          ...
          <div className="absolute -bottom-6 -left-2 sm:-bottom-10 sm:-left-6 bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-50 hidden sm:block animate-bounce-slow">
            <p className="text-3xl sm:text-4xl font-black text-blue-600">5+</p>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Yillik Tajriba</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-6 text-center md:text-left">
          <div className="space-y-2">
            <h2 className="text-3xl sm:text-4xl font-black text-slate-900">{t('nav.projects')}</h2>
            <p className="text-slate-500 font-medium italic">Oxirgi amalga oshirilgan ijodiy ishlarim</p>
          </div>
          <Link to="/projects" className="px-6 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:text-blue-600 hover:border-blue-600 transition-all shadow-sm">
            Barcha loyihalar →
          </Link>
        </div>
